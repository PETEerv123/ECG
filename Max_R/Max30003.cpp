...
MAX30003::MAX30003(int cs_pin) 
    : notchFilter(7, 512.0, 20) { // f0=5Hz, fs=512Hz, Q=20
    _cs_pin = cs_pin;
    pinMode(_cs_pin, OUTPUT);
    digitalWrite(_cs_pin, HIGH);
}
...
void MAX30003::max30003Begin() {
  ...
  max30003RegWrite(CNFG_GEN, 0x8055B);  // Dòng điện DC Lead-Off: 100nA, ngưỡng phát hiện: VMID ± 300mV Bia 200M
  delay(100);
  ...
  max30003RegWrite(CNFG_ECG, 0x825000); //
  delay(100);
  // 512Hz, Gain=40, DLPF=40Hz, DHPF=0.5Hz //0x815000
  // Cấu hình CNFG_RTOR1
  max30003RegWrite(CNFG_RTOR1, 0x3FC600);  //0x5FC600 0x3fc600 0x3FA300
  delay(100);
  // Đồng bộ hóa cài đặt
  max30003Synch();
  delay(100);
